...
cudaError_t SM4_cuda_ecb(char *text,char *cipher,int size,muint32 *key){
	...
	clock_t end=clock();
	printf("Encryption/Dencryption kernels in GPU alone takes %d/%d seconds.\n",end-start,CLOCKS_PER_SEC);
...
int main(){
	...
	////读取文件内容
    FILE* file;
	char filename[]="C:\\Users\\abcd\\Desktop\\ptext.txt";
	...
	printf("NOW ECB...\n");
	clock_t start=clock();
	cudaError_t cudaStatus=SM4_cuda_ecb(p,m,size,rk);
	if(cudaStatus!=cudaSuccess){
		printf("SM4_cuda_ecb failed\n");

	}
	clock_t end=clock();
	printf("ECB Encryption takes %d/%d seconds.\n",end-start,CLOCKS_PER_SEC);
	...
	printf("ECB Dencryption takes %d/%d seconds.\n",end-start,CLOCKS_PER_SEC);
	//输出还原的明文
	printf("ECB模式解密后:\n");
	...
	printf("CTR Encryption takes %d/%d seconds.\n",end-start,CLOCKS_PER_SEC);
	...
	printf("CTR Dencryption takes %d/%d seconds.\n",end-start,CLOCKS_PER_SEC);
	...
	/////////////////////CBC////////////////////////
	//CBC 没有用cuda并行，因为后一个明文块需要和前一块密文块异或后才能加密    
